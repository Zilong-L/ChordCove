...
const midiRange = (startNote: string, endNote: string) => {
    ...
    return Array.from({ length: endMidi - startMidi + 1 }, (_, i) => startMidi + i)
        .map((midi) => ({ name: Note.fromMidi(midi)!, midi }));
};
...
export default function KeySelector() {
    ...
    return (
        <Transition show={true}>
            <div className=" grid-flow-row grid grid-cols-[80px_50px]">
                Key:
                <Listbox value={selectedKey} onChange={handleKeyChange}>
                    <div className="relative w-32">
                        <ListboxButton className="w-full text-white px-2 rounded text-left">{selectedKey}</ListboxButton>
                        <ListboxOptions className="absolute mt-1 max-h-60 w-full overflow-auto text-white rounded shadow-lg backdrop-blur-md z-50">
                            {notes.map((note) => (
