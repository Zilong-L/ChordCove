...
export async function addLocalSheet(initialData: {
  localKey: string;
  metadata: Partial<Omit<LocalSheetMetadata, "localKey" | "serverModifiedAt" | "localLastSavedAt">>;
...
export async function updateLocalSheetMetadata(
  localKey: string,
  updates: Partial<Omit<LocalSheetMetadata, "localKey" | "serverModifiedAt" | "localLastSavedAt">>
): Promise<void> {
  const db = await getDB();
